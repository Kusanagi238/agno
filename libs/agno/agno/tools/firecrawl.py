...
try:
    ...
    # Try a fallback symbol name used by some versions of firecrawl
    try:
        from firecrawl import FirecrawlApp  # type: ignore[attr-defined]
        from firecrawl import V1ScrapeOptions as ScrapeOptions
    except Exception as e2:
        raise ImportError(
            f"`firecrawl-py` import failed. Tried importing 'ScrapeOptions' and 'V1ScrapeOptions'. Original errors: {e!r}; {e2!r}. Please install or upgrade `firecrawl-py` (pip install firecrawl-py)"
        ) from e2
