...
try:
    ...
    # Try fallback name used by older/newer versions of the package
    try:
        from firecrawl import FirecrawlApp  # type: ignore[attr-defined]
        from firecrawl import V1ScrapeOptions as ScrapeOptions
    except ImportError:
        # Re-raise with original exception context so the true cause is visible
        raise ImportError(
            f"Failed to import required symbols from 'firecrawl': {e}. If 'firecrawl-py' is not installed, install it with `pip install firecrawl-py`."
        ) from e
