...
def get_workspace_dir_path(ws_root_path: Path) -> Path:
    ...
    if ws_pyproject_toml.exists() and ws_pyproject_toml.is_file():
        ...
        if agno_conf is not None:
            agno_conf_workspace_dir = agno_conf.get("workspace", None)
            # Ensure we don't pass None (or other unexpected types) to Path.joinpath
            if agno_conf_workspace_dir is not None:
                agno_conf_workspace_dir_str = str(agno_conf_workspace_dir)
